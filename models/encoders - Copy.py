...
class Encoder(nn.Module):
    ...
    def encode(self, x, adj):
        if self.encode_graph: ## just changes to tuple? i don't see the difference
            input = (x, adj)
...
class HGCN(Encoder):
    ...
    def __init__(self, c, args):
        ...
        self.manifold = getattr(manifolds, args.manifold)()
        print(self.manifold,'manifold for encoder')
        assert args.num_layers > 1
        dims, acts, self.curvatures,use_acts = hyp_layers.get_dim_act_curv(args)
        print(args.dim,args.output_dim,'dims')
        print(dims,'dim',acts,'acts full')
        # print(dims[0].shape,'dim shape',acts[0].shape,'acts shape')
        self.curvatures.append(self.c)
        hgc_layers = []
        for i in range(len(dims) - 1):  #### why dims not layers?? ### don't be a dumdum, it just goes through # of sets of dims not the dims themselfs
            ### but.... why is this len dims -1? ## bc think- 3 dims means input, second, output- we only need two transformations for that
            c_in, c_out = self.curvatures[i], self.curvatures[i + 1]
            in_dim, out_dim = dims[i], dims[i + 1]
            act = acts[i]
            use_act = use_acts[i] ### bc we linear lamba is throwing errors, don't change final act and just pass use_act properly
            hgc_layers.append(
                    hyp_layers.HyperbolicGraphConvolution(
                            self.manifold, in_dim, out_dim, c_in, c_out, args.dropout, act, args.bias, args.use_att, args.local_agg,use_act
                    )
            )


        # print(hgc_layers[0],'first dime layer')
        # print(len(hgc_layers),'how many layers?')
        self.layers = nn.Sequential(*hgc_layers)
        print(self.layers,'layer shape')
        self.encode_graph = True
        # ddd

    def encode(self, x, adj):
        # print(x.type(),adj.type(),'encoding type')
