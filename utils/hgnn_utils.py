...
def th_atanh(x, EPS,args):
	values = th.min(x, th.Tensor([1.0 - EPS]).to(args.device))
...
def set_up_optimizer_scheduler(hyperbolic, args, model):
	if hyperbolic:
		...
		print(len(list(model.parameters())),len(hyperbolic_params),len(euclidean_params))
		# print(list(model.parameters()),hyperbolic_params,euclidean_params)
...
def get_all_data(data_loader):
	...
	for i_batch, sample_batched in enumerate(data_loader):
		# print(sample_batched['adj_mat'].shape,'sampled')
		max_len =  sample_batched['adj_mat'].shape[-1] if sample_batched['adj_mat'].shape[-1] > max_len else max_len
		sampled+=1
	...
	for i_batch, sample_batched in enumerate(data_loader):
		...
		matr =sample_batched['weight']
		# print(matr)
		# print(sample_batched['adj_mat'])
		# print(matr.shape,'MAT')
		for i in range(matr.shape[-1]):
		# for r in sample_batched:
			# print()
			r = sample_batched['adj_mat'][:,:,i]
			b = sample_batched['weight'][:,:,i]
			err
			# print(r)
			# print(b)
			# remaining = max_len-len(r)
			# row = r+[[0]*remaining]
			# new_row.extend(row)
		xs.append(new_row)
	print(xs)
...
def baseline_scores(args,train_loader,dev_loader,test_loader):
	train_data = get_all_data(train_loader)

	# print(train_data)

	dev_data = get_all_data(dev_loader)
	test_data = get_all_data(test_loader)
